  ...
  return (
    <form onSubmit={enviar} className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
      <label className="block sm:col-span-2">
        <span className="mb-1.5 block text-sm font-semibold text-ink-soft">Nome completo *</span>
        <input className={inp} value={f.nome} onChange={set('nome')} autoComplete="name" required />
      </label>
      <label className="block">
        <span className="mb-1.5 block text-sm font-semibold text-ink-soft">E-mail</span>
        <input type="email" inputMode="email" autoComplete="email" className={inp} value={f.email} onChange={set('email')} />
      </label>
      <label className="block">
        <span className="mb-1.5 block text-sm font-semibold text-ink-soft">Telefone</span>
        <input type="tel" inputMode="tel" autoComplete="tel" className={inp} value={f.telefone} onChange={set('telefone')} />
      </label>
      <label className="block sm:col-span-2">
        <span className="mb-1.5 block text-sm font-semibold text-ink-soft">Link do currículo</span>
      ...
      <label className="block sm:col-span-2">
        ...
        <textarea className={`${inp} min-h-[88px]`} value={f.mensagem} onChange={set('mensagem')} placeholder="Conte por que você é um bom match para a vaga." />
      </label>
      {/* honeypot (oculto) */}
