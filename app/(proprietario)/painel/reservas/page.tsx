...
export default function ReservasPage() {
  ...
  useEffect(() => {
    ...
    if (p === 'conectado') { flash('✅ Mercado Pago conectado! Você já pode receber pagamentos.'); window.history.replaceState({}, '', '/painel/reservas') }
    else if (p === 'erro') { flash('Não foi possível conectar o Mercado Pago. Tente novamente.'); window.history.replaceState({}, '', '/painel/reservas') }
          ...
          <div className="space-y-12">
            ...
            {props.length > 0 && (
              <section>
                ...
                <p className="text-sm text-ink-muted mb-4">O bloqueio de datas e os preços por dia agora ficam no Calendário, junto com as reservas confirmadas — um só lugar para a agenda do espaço.</p>
                <Link href="/painel/calendario" className="inline-flex items-center gap-2 rounded-xl bg-ink hover:bg-ink-soft text-white font-bold text-sm px-5 py-2.5 transition-colors">📅 Gerenciar disponibilidade no Calendário →</Link>
              </section>
            )}
          </div>
        )}
